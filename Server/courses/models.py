...
class Subchapter(models.Model):
    ...
    def get_edit_lessons_url(self):
        return reverse("courses:edit_content_of_subchapter", kwargs={"subchapter_pk": self.pk})

    def get_view_lessons_url(self):
...
class Lesson(models.Model):
    ...
    theme = models.CharField(max_length=255, verbose_name="Тема урока", null=True, blank=True)
    slug = models.SlugField(unique=False, blank=True, null=True)
...
class LessonContent(models.Model):
    ...
    def __str__(self):
        return f"{self.content_type.name} № {self.object_id} для урока {self.lesson.theme.lower()}"
...
class Image(models.Model):
    file = models.ImageField(upload_to='course_images')
    ...
    def __str__(self):
        return f"Изображение {self.file.name}"
...
class Video(models.Model):
    ...
    def __str__(self):
        return f"Видео по ссылке {self.url}"
